...
function heart (x, y, size) {
  ...
  myScaledCanvas.vertex(x, y);
  myScaledCanvas.bezierVertex(x-size/2, y-size/2, x-size, y+size/3, x, y+size);
  myScaledCanvas.bezierVertex(x+size, y+size/3, x+size/2, y-size/2, x, y);
...
function drawMyDesign() {
  ...
  myScaledCanvas.background('rgb(255,255,255)'); // white
  // myScaledCanvas.background('rgb(0,0,0)'); // black
  ...
  myScaledCanvas.push();
  myScaledCanvas.translate(-60 + WIDTH/2, (-60 + HEIGHT/2));
  ...
  for (var i = 0; i < floor(npoints*1.3) + 1; i += 1) {
    ...
    myScaledCanvas.scale(1.6*(i+1)/npoints);
